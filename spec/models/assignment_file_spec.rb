...
describe AssignmentFile do
  ...
  it "removes spaces from file names on save" do
    @assignment_file.file = fixture_file('Spaces In Name.jpg', 'img/jpg')
    @assignment.save!
    expect @assignment_file.url.should =~ /.*\/uploads\/assignment_file\/file\/#{@assignment_file.id}\/\d+_spaces_in_name\.jpg/
  end
end
